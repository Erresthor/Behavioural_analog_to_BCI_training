...
def _transpose_list(_list):
    ...
    if type(_list[0])==dict:
        ...
        for key in _list[0].keys():
            transposed_dict[key] = [el[key] for el in _list]
        return transposed_dict
    
    transposed_once = list(zip(*_list))
...
def run_loop(environment,agent_functions,seed,Ntrials,verbose=True):
    ...
    for trial in range(Ntrials):
        ...
        while not end_trial:
            ...
            states.append(state)
            actions.append(u_vec)  # This is a dict of vec now !
        ...
        ref_observations = _transpose_list(observations)
        ref_states = _transpose_list(states)        
        ref_actions = _transpose_list(actions)
        ...
        full_history["actions"].append(ref_actions)
...
class TrainingEnvironment :
    ...
    def step(self,action_chosen):
        # action_chosen is a dictionnary of 3 action dimensions :         
        angle_action = action_chosen["angle"]
        distance_action = action_chosen["distance"]
        
        # The effective action is the one that will affect the system. It is driven 
        # by the angle, except whenn the distance between the points is too small :     
        ...
        [s_d,s_idx,s_vect],[o_d,o_idx,o_vect] = process_update(timestep_rngkey,self.current_state,self.vec_a,self.vec_b,effective_action)
...
def generate_synthetic_data(environment_object,agent_functions,n_trials=10,seed=0,verbose=True):
    ...
    formatted_stimuli= [jnp.array(o) for o in _transpose_list(training_hist["stimuli"])]
    bool_stimuli = [jnp.ones_like(stim[...,0]) for stim in formatted_stimuli]
    rewards = jnp.array(training_hist["rewards"])
    
    # Note the change here : 
    actions = {key: jnp.array(value) for key, value in _transpose_list(training_hist["actions"]).items()}
